...
<div class="table-responsive">
    <table id="example1" class="table table-striped table-bordered table-condensed table-hover display no-wrap cell-border" cellspacing="0" width="100%">
        ...
        <tbody style="font-size: 15px" id="list-factura" name="list-factura">
            @foreach($facturas as $key => $factura)
            <tr id="recibo-{{ $factura->id }}">
                <td style="width:8%;" valign="middle" align="center" title="{{ $factura->comentarios }}" data-tooltip="tooltip">
                    @if($factura->estatus == 'Pagado')
                    <label class="label label-success" style="font-size: 15px">{{ $factura->folio_recibo }}</label>
                    @elseif($factura->estatus == 'Cancelado')
                    <label class="label label-danger" style="font-size: 15px">{{ $factura->folio_recibo }}</label>
                    @elseif($factura->estatus == 'Pendiente')
                    <label class="label label-warning" style="font-size: 15px" data-target="#modal-pagar-recibo" data-toggle="modal" onclick="PagarRecibo({{ $factura->id }}, {{ $factura->folio_recibo }}, {{ $factura->saldo }}, 'Recibo')">{{ $factura->folio_recibo }}</label>
                    @endif
                </td>
                ...
                <td style="width:12%;" align="center">
                    <div class="btn-group">
                        ...
                        @elseif($factura->detalles > 0)
                            ...
                            onclick="MostrarDetallesFactura({{ $factura->id }}, {{ $factura->folio_recibo }}, 'Recibo')"
                            >
                                <i class="glyphicon glyphicon-th-list"></i>
                                <span class="label label-success">{{ $factura->detalles }}</span> 
                            </a>
                        @endif
                        <a class="btn btn-sm btn-warning" onclick="EditRecibo({{ $factura->id }}, 'Pendiente')" data-toggle="modal" data-target="#modal-recibo" >
