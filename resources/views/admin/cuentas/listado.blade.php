    ...
    <tbody style="font-size: 15px" id="list" name="list">
        ...
        <tr id="cuenta-{{ $cuenta->id }}">
            ...
            <td style="width:10%;" align="center">
                <a class="btn btn-xs btn-warning" data-toggle="modal" data-target="#modal-cuenta" onclick="Edit({{ $cuenta->id }})" title="Editar cuenta {{ $cuenta->alias }}" data-tooltip="tooltip">
                    <i class="fas fa-edit"></i>
                </a>
                @if($cuenta->estatus == 1)
                    <a class="btn btn-xs btn-danger" title="Inactivar cuenta {{ $cuenta->alias }}" data-tooltip="tooltip" onclick="Inactivar({{ $cuenta->id }})">
                        <i class="fas fa-eye-slash"></i>
                    </a>
                @else
                    <a class="btn btn-xs btn-success" title="Activar cuenta {{ $cuenta->alias }}" data-tooltip="tooltip" onclick="Activar({{ $cuenta->id }})">
