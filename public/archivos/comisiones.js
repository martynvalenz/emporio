...
$('#estatus_cobranza_select').change(function()
{
    Listar();
});
...
function ListarAsignacion()
{
    ...
    if (buscar == '')
    {
        ...
        {
            type: 'get',
            url: '/admin/comisiones-asignacion',
            success: function(data)
            {
                $('#listado-asignacion').empty().html(data);
                $(".tooltip").tooltip("hide");
                $(function()
                {
                    $('#example2').stickyTableHeaders();
                });
            }
        });
    }
    else
    {
        ...
        {
            type: 'get',
            url:  '/admin/comisiones-asignacion-buscar/' + buscar,
            success: function(data)
            {
                //console.log(data)
                $('#listado-asignacion').empty().html(data);
...
$(document).on("click", ".pagination-asignacion .pagination li a", function(e)
{
    e.preventDefault();
    var url = $(this).attr("href");
    $.ajax(
...
//Dirección
function EditarComision(id, monto)
{
