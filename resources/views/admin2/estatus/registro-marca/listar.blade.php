    ...
    <table id="example1" class="table table-striped table-bordered table-condensed table-hover display table-responsive no-wrap cell-border" cellspacing="0" width="100%">
        ...
        <tbody style="font-size: 15px" id="list" name="list">
            ...
            <tr id="bitacora-{{ $bitacora->id }}">
                ...
                <td style="width:15%;" valign="middle" data-tooltip="tooltip"><i class="fas fa-edit"></i></a> {{ $bitacora->marca }}</td>
                ...
                @if($bitacora->comprobacion_uso == 0)
                    <td style="width:9%;" align="center" valign="middle" title="Comprobación de uso: {{ Carbon\Carbon::parse($bitacora->fecha_comprobacion_uso)->format('d-m-Y') }}" data-tooltip="tooltip"><i class="fas fa-times" style="color: red"></i> {{ Carbon\Carbon::parse($bitacora->fecha_comprobacion_uso)->diffInDays() }} días</td>
                ...
                <td style="width:12%;" align="center">
                    <a class="btn btn-xs btn-default btn-flat btn-comentarios-modal" onclick="Comentarios({{ $bitacora->id }}" data-target="#comentarios-modal" data-toggle="modal" data-tooltip="tooltip" title="Comentarios">
                        <i class="glyphicon glyphicon-list-alt"></i>
                    </a>
                    <a class="btn btn-info btn-xs" title="Contactos" data-tooltip="tooltip"><i class="fas fa-users"></i></a>
                    <a class="btn btn-warning btn-xs" title="Editar {{ $bitacora->id }}" data-tooltip="tooltip" data-toggle="modal" data-target="#modal-editar-{{ $bitacora->id }}"><i class="fas fa-edit"></i></a>
                    @if($bitacora->estatus == 'Cancelado')  
                        <a class="btn btn-success btn-xs" title="Activar" data-tooltip="tooltip" data-target="#modal-activar-{{ $bitacora->id }}" data-toggle="modal"><i class="fas fa-check"></i></a>
                    @else
                        <a class="btn btn-danger btn-xs" title="Cancelar" data-tooltip="tooltip" data-target="#modal-cancelar-{{ $bitacora->id }}" data-toggle="modal"><i class="fas fa-times"></i></a>
                    @endif

                    <a class="btn btn-primary btn-xs" title="Crear un servicio" data-tooltip="tooltip"><i class="fas fa-external-link-square-alt"></i></a>
                </td>
            </tr>
            @endforeach
        </tbody>
