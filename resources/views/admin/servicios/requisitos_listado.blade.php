    ...
    <tbody id="list" name="list">
        ...
        <tr id="listado-{{ $listado->id }}">
            ...
            <td style="width:10%; font-weight: bold;" align="center"> 
                ...
                @else
                    <a class="btn btn-success btn-xs" onclick="SubirOrden({{ $listado->id }}, {{ $listado->orden }})"><i class="fas fa-arrow-up"></i></a> 
                @endif
                @if($listado->orden >= $ultimo_orden->orden)
                    <a class="btn btn-warning btn-xs disabled" disabled><i class="fas fa-arrow-down"></i></a>
                @else
                    <a class="btn btn-warning btn-xs" onclick="BajarOrden({{ $listado->id }}, {{ $listado->orden }})"><i class="fas fa-arrow-down"></i></a>
                @endif
            </td>
            ...
            <td style="width:19%;" align="center">
                <div class="btn-group">
                    <a class="btn btn-warning btn-xs" title="Libera comisión de Venta" data-tooltip="tooltip" onclick="LiberarComisiones(1, {{ $listado->id }}, {{ $listado->id_catalogo_servicio }})"><i class="fas fa-money-bill-alt"></i></a>
                    <a class="btn btn-pink btn-xs" title="Libera comisión Operativa" data-tooltip="tooltip" onclick="LiberarComisiones(2, {{ $listado->id }}, {{ $listado->id_catalogo_servicio }})"><i class="fas fa-money-bill-alt"></i></a>
                    <a class="btn btn-green btn-xs" title="Libera comisión por Gestión" data-tooltip="tooltip" onclick="LiberarComisiones(3, {{ $listado->id }}, {{ $listado->id_catalogo_servicio }})"><i class="fas fa-money-bill-alt"></i></a>
                    <a class="btn btn-primary btn-xs" title="Registra trámite" data-tooltip="tooltip" onclick="LiberarComisiones(4, {{ $listado->id }}, {{ $listado->id_catalogo_servicio }})"><i class="far fa-registered"></i></a>
                </div>
                
                <i style="padding-left: 1.2em"></i>
                <a class="btn btn-danger btn-xs" title="Quitar paso: {{ $listado->requisito }}" data-tooltip="tooltip" onclick="QuitarRequisito({{ $listado->id }}, {{ $listado->id_catalogo_servicio }})"><i class="fas fa-times"></i></a>
            </td>
        </tr>
        @endforeach
    </tbody>
...
@else
    <h4>No hay registros encontrados, seleccione procesos del listado superior.</h4>
@endif
