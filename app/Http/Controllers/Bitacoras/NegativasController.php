...
class NegativasController extends Controller
{
    public function index()
    {
        ...
        $url_listar = '/admin/bitacora/negativas-listar/';
        $url_buscar = '/admin/bitacora/negativas-buscar/';
        $url_actualizar = '/admin/bitacora/negativas/actualizar-servicio/';

        return view('admin.bitacoras.negativas.index', compact('monedas', 'admins', 'porcentaje_iva', 'cuentas', 'formas_pago', 'variable_estatus', 'clases', 'bitacoras', 'estrategias', 'url_listar', 'url_buscar', 'fecha_inicio', 'fecha_fin', 'tipo_vista', 'bitacoras_estatus', 'url_actualizar', 'today'));
    }
    ...
    public function listar($estatus, $fecha_inicio, $fecha_fin)
    {
        if($estatus == 'todos')
        {
            ...
            return view('admin.bitacoras.negativas.listado', compact('servicios', 'today'));    
        }
        else
        {
            Carbon::setLocale('es');
    ...
    public function buscar($estatus, $buscar, $fecha_inicio, $fecha_fin)
    {
        ...
        else
        {
            ...
            return view('admin.bitacoras.negativas.listado', compact('servicios', 'facturas', 'today'));
        }
    }
    ...
    public function actualizarServicio($id_servicio)
    {
        ...
        return view('admin.bitacoras.negativas.listado-servicios', compact('servicio', 'today'));
    }
    ...
    public function elaboracion_notificacion(Request $request, $id)
    {
        $servicio = Servicios::findOrFail($id);
    ...
    public function envio_notificacion(Request $request, $id)
    {
        $servicio = Servicios::findOrFail($id);
        ...
        if($request->respuesta_cliente != '')
        {
            $this->validate($request,
                [
                    'respuesta_cliente_fecha' => 'required'
                ]);

            $servicio->respuesta_cliente_boolean=1;
            $servicio->respuesta_cliente=$request->respuesta_cliente;
            $servicio->respuesta_cliente_fecha=$request->respuesta_cliente_fecha;
        }
        else
        {
            $servicio->respuesta_cliente_boolean=0;
            $servicio->respuesta_cliente=$request->respuesta_cliente;
            $servicio->respuesta_cliente_fecha=null;
        }

        $servicio->respuesta_cliente_comentarios=$request->respuesta_cliente_comentarios;

        if($request->respuesta_cliente_comentarios != '' && $request->respuesta_cliente_comentarios != $request->respuesta_cliente_comentarios_value)
        {
            $comentario = DB::table('servicios_comentarios')
                ->insert(
                    [
                        'comentario' => 'Respuesta del cliente: '.$request->respuesta_cliente_comentarios,
