        ...
        <table class="table m-b-0 table-hover table-striped table-bordered" cellspacing="0" width="100%">
            ...
            <tbody>
                ...
                @foreach($servicios  as $servicio)
                    <tr id="servicio-recibo-{{ $servicio->id }}">
                        ...
                        <td style="width:10%;" align="center">
                            <div class="checkbox checkbox-css checkbox_servicio_recibo_div">
                                <input type="checkbox" class="checkbox_servicio" id="servicio-recibo-pendiente-{{ $servicio->id }}" onclick="RecibirServicio({{ $servicio->id }})" @if($servicio->id_det != '') checked value="1" @else unchecked value="0" @endif />
                                <label for="servicio-recibo-pendiente-{{ $servicio->id }}"></label>
                            </div>
                            <input type="hidden"  @if($servicio->id_det != '') checked value="1" @else unchecked value="0" @endif id="servicio-recibo-pendiente-val-{{ $servicio->id }}">
                            <div class="checkbox checkbox-css checkbox_servicio_recibo_pagado">
            ...
            <tfoot>
                <tr>
                    <th colspan ="5">&nbsp;</th>
                    <th>Pendiente: </th>
                    <th style="font-weight: bold; text-align: right;" align="right" id="recibo_pendiente">
                        @if($monto_pendiente == '') 0 @else {{ number_format($monto_pendiente->suma_total,2) }} @endif</th>
                    <th colspan ="1">&nbsp;</th>
                </tr>
                <input type="hidden" id="recibo_pendiente_val" @if($monto_pendiente == '') value="0" @else value="{{ $monto_pendiente->suma_total }}" @endif>
            </tfoot>
        </table>
    </div>
</div>
