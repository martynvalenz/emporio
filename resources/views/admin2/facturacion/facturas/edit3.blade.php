        ...
        <form role="form" id="edtiarForm" action="{{ route('facturas.update', $factura->id) }}" method="post" enctype="multipart/form-data">
            ...
            @if(count($servicios_seleccionar) > 0)
              ...
              <div class="col-lg-12 col-md-12 col-sm-12 col-xs 12">
                <div class="table-responsive">
                  <table id="servicios-pendientes" class="table table-striped table-bordered table-hover table-responsive" style="font-size: 16px">
                        ...
                        <tr id="servicio{{ $servicio->id }}">
                          ...
                          <td style="width:5%;" align="center">
                            <button 
                              ...
                              data-id="{{ $servicio->id }}"
                              data-path="{{ route('facturas.insertar-detalle') }}"
          ...
          <div class="row">
            @if(count($detalles) > 0)
              ...
              <div class="col-lg-12 col-md-12 col-sm-12 col-xs 12">
                <div class="table-responsive">
                  <table id="servicios-facturados" class="table table-striped table-bordered table-hover table-responsive" style="font-size: 16px">
                    ...
                    <tbody>
                      @foreach($detalles as $det)
                        <tr id="det{{ $det->id }}">
                          <td style="width:10%;" valign="middle" align="left">{{ $det->Servicios->id }}</td>
                          <td style="width:10%;" valign="middle" align="left" title="{{ $det->Servicios->servicio }}" data-tooltip="tooltip">
                            {{ $det->Servicios->CatalogoServicios->clave }}
                          </td>
                          <td style="width:40%;" valign="middle" align="left">{{ $det->Servicios->tramite }} {{ $det->Servicios->Control->nombre }} {{ $det->Servicios->clase }}</td>
                          <td style="width:10%;" align="center" valign="middle" title="Agregado {{ Carbon\Carbon::parse($det->Servicios->created_at)->diffForHumans() }}" data-tooltip="tooltip">{{ Carbon\Carbon::parse($det->Servicios->created_at)->format('d/m/Y') }}</td>
                          <td style="width:10%;" valign="middle" align="right" data-tooltip="tooltip">{{ number_format($det->monto,2) }}</td>
                          <td style="width:10%;" valign="middle" align="right">{{ number_format($det->Servicios->costo - $det->Servicios->facturado,2) }}</td>
...
<script>
  ...
  $(document).ready(function() 
  {
    ...
    //ajax
    $.get('/admin/facturas/razones/' + select_cliente, function(data)
    {
        console.log(data);
    ...
    $(".btn-insertar").on('click', function(e)
    {
